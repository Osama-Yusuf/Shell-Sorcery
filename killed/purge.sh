...
if [ -z $1 ] || [ $1 == '-h' ] || [ $1 == '--help' ]; then
    ...
    echo "  -ct, --created    remove all created containers (the created and not running ones)"
...
elif [ $1 == '-ct' ] || [ $1 == '--created' ]; then
    echo
    echo "Deleting all created containers"
    conts=$(docker ps -a | grep 'Created' | awk '{print $1}')
    if [ -n "$conts" ]; then
        docker rm -f $conts
    else
        echo
        echo "No created containers"
    fi
else
    echo
    echo "Invalid option"
    echo "Try 'clean_none.sh --help' for more information"
fi
