...
select chosen_namespace in "${namespaces_array[@]}"; do
    ...
    if [[ -n "$chosen_namespace" ]]; then
        echo -e "\nYou have selected the $chosen_namespace namespace"
...
select microservice in "${microservices_array[@]}"; do
    ...
    if [[ -n "$microservice" ]]; then
        echo -e "\nYou have selected the $microservice microservice\n"
        break # Break the loop if selection is valid
    else
        echo "invalid" # Indicate invalid selection
    fi
...
do
    ...
    # image should be like this: 
        # your_img_repo.com:5000/group_name/project_name/release:<CI_COMMIT_SHORT_SHA>
    validation_output=$(kubectl describe pod $microservice_pod -n $chosen_namespace | grep -i " image:" | grep release: | cut -d ":" -f4)
